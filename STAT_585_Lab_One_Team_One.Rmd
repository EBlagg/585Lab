...
weather1 <- read_fwf(file.path(dataDir,"ushcn-v2.5-stations.txt"), 
...
```{r echo = FALSE}
# Change elevation to be a numeric rather than factor
weather2 <- weather1 %>%
  ...
  mutate(time_zone = forcats::fct_recode(time_zone, 
                                         "ETZ" = "5", "CTZ" = "6", 
                                         "MTZ" = "7", "PTZ" = "8")) %>%
  mutate(time_zone = forcats::fct_relevel(time_zone, "CTZ", "ETZ", "MTZ", "PTZ"))
```
...
ggplot() + 
  geom_polygon(data = usa, 
               ...
               alpha = 0.3,
               color = "black") +
  geom_point(data = weather2,
             aes(x = longitude, y = latitude, colour = time_zone, alpha = elevation)) +
  coord_fixed(1.3) + 
  scale_fill_manual(values = c("#3C3C3C", "#7896B4", "#E6E6DC", "#FAAA72"),
                    name = "Time Zone") +
  scale_color_manual(values = c("#3C3C3C", "#7896B4", "#E6E6DC", "#FAAA72"),
                     name = "Time Zone") + 
  labs(alpha = "Elevation") +
  theme_bw()
```
